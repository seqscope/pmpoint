...
int32_t cmd_export_polygon_pmtiles(int32_t argc, char **argv)
{
    ...
    std::string meta_layer_name = "vector_layers";
    std::string remove_columns = "lat,lon";
    std::string missing_value = "NA";
    std::string columns_str;
    std::string colname_x_centroid = "X";
    std::string colname_y_centroid = "Y";
    std::string colname_vertices = "Vertices";
    ...
    LONG_STRING_PARAM("columns", &columns_str, "Columns to extract from the metadata (comma-separated)")
    LONG_STRING_PARAM("layer-name", &meta_layer_name, "Layer name in the metadata (default: vector_layers)")
    LONG_STRING_PARAM("remove-columns", &remove_columns, "Columns to remove from the output (comma-separated, default: lat,lon)")
    LONG_STRING_PARAM("missing-value", &missing_value, "String for missing value (default: NA)")

    LONG_PARAM_GROUP("Output options", NULL)
    LONG_STRING_PARAM("out-tsv", &out_tsvf, "Output TSV file")
    //LONG_STRING_PARAM("out-json", &out_jsonf, "Output JSON file")
    LONG_PARAM("write-vertices", &write_vertices, "Write vertices of the polygon (default: false)")
    ...
    if (out_tsvf.empty())
    {
        error("Missing required options --out-tsv");
    }
    ...
    // Access the metadata information to obtain the column names
    std::vector<std::string> col_names;
    if (columns_str.empty()) {    
        if (pmt.jmeta.count(meta_layer_name)) {
            //notice("Getting layer %s", meta_layer_name.c_str());
            const auto& meta_layers = pmt.jmeta.at(meta_layer_name);
            //notice("Getting fields..");
            const auto& meta_layer_fields = meta_layers[0].at("fields");
           // notice("Iterating each field..");
           std::vector<std::string> remove_cols;
           split(remove_cols, ",", remove_columns);
           std::set<std::string> remove_set(remove_cols.begin(), remove_cols.end());
            for (const auto& element : meta_layer_fields.items()) {
                if (remove_set.count(element.key())) {
                    continue;
                }
                col_names.push_back(element.key());
            }
            notice("Found %d columns in the metadata", col_names.size());
            for (const auto& col_name : col_names) {
                notice("Column name: %s", col_name.c_str());
            }
        }
        else {
            error("Metadata not found for layer %s", meta_layer_name.c_str());
        }
    } else {
        split(col_names, ",", columns_str);
    }
    ...
    // (d) (aEY-bE only) pass
    polygon_dataframe df;
    df.set_columns(col_names);
    mvt_polygons_filt mvtfilt(&df);
    ...
    for (int32_t i = 0; i < pmt.tile_entries.size(); ++i)
    {
        ...
        if (tsv_wh != NULL)
        {
            if (!tsv_hdr_written)
            {
                if (df.polygons.size() > 0)
                {
                    hprintf(tsv_wh, "%s\t%s", colname_x_centroid.c_str(), colname_y_centroid.c_str());
                    if (write_vertices) {
                        hprintf(tsv_wh, "\t%s", colname_vertices.c_str());
                    }
                    for (int32_t i = 0; i < df.feature_columns.size(); ++i)
                    {
                        hprintf(tsv_wh, "\t%s", df.feature_columns[i].c_str());
                    }
                    hprintf(tsv_wh, "\n");
                    tsv_hdr_written = true;
                }
            }
            for (int32_t i = 0; i < df.polygons.size(); ++i)
            {
                //notice("i = %d", i);
                std::vector<pmt_utils::pmt_pt_t>& pts = df.polygons[i].points;
                double gx = 0, gy = 0;
                ...
                for (int32_t j = 0; j < df.feature_columns.size(); ++j)
                {
                    //notice("j = %d", j);
                    std::string& s = df.feature_matrix[i][j];
                    if ( s.empty() ) {
                        hprintf(tsv_wh, "\t%s", missing_value.c_str());
                    } else {
                        hprintf(tsv_wh, "\t%s", s.c_str());
                    }
                }
                hprintf(tsv_wh, "\n");
                //notice("Wrote %d points", i+1);
            }
        }
        // if (json_wh != NULL)
        // {
